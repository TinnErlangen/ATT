...
for run in runs:
    ...
    stc_m.plot(figure=fig_idx,subjects_dir=subjects_dir,subject="nc_"+subject,
    ...
    stc_t.plot(figure=fig_idx,subjects_dir=subjects_dir,subject="nc_"+subject,
    ...
    stc_tm.plot(figure=fig_idx,subjects_dir=subjects_dir,subject="nc_"+subject,
    hemi="both", clim=clim, smoothing_steps = smoothing_steps)
    mlab.title(run + " mean, t-masked")
    fig_idx += 1

    fwd = mne.read_forward_solution("../proc/nc_"+subject+"_"+run+"_ica-fwd.fif")
    ...
    lims=[0, 0.5, 1]), smoothing_steps = smoothing_steps)
    mlab.title(run + " forward sensitivity")
    fig_idx += 1
