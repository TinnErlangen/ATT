...
vmin, vmax = -3., 3.
mplcol = MplColorHelper("seismic", vmin, vmax)
...
formula = "DICS_log ~ dPTE*C(Reg, Treatment('L8143-rh'))*Task"
model = MixedLM.from_formula(formula, df, groups=df['Subj'],
                             re_formula=None)
dPTE_DICS_fit = model.fit(method=["powell", "lbfgs"])
...
# overall
intercept = dPTE_DICS_fit.params["Intercept"]
fx = dPTE_DICS_fit.params["dPTE"]
for idx, label in enumerate(labels):
    lab = label.name
    vals[idx] = intercept + fx
    if lab != "L8143-rh":
        if dPTE_DICS_fit.pvalues[f"C(Reg, Treatment('L8143-rh'))[T.{lab}]"] < .05:
            reg_fx = dPTE_DICS_fit.params[f"C(Reg, Treatment('L8143-rh'))[T.{lab}]"]
            vals[idx] += reg_fx
...
# task
fx_task = dPTE_DICS_fit.params["dPTE:Task[T.task]"]
vals = np.zeros(len(labels))
for idx, label in enumerate(labels):
    lab = label.name
    vals[idx] = intercept + fx + fx_task
    if lab != "L8143-rh":
        if dPTE_DICS_fit.pvalues[f"C(Reg, Treatment('L8143-rh'))[T.{lab}]:Task[T.task]"] < .05:
            reg_fx = dPTE_DICS_fit.params[f"C(Reg, Treatment('L8143-rh'))[T.{lab}]:Task[T.task]"]
            vals[idx] += reg_fx
        if dPTE_DICS_fit.pvalues[f"dPTE:C(Reg, Treatment('L8143-rh'))[T.{lab}]:Task[T.task]"] < .05:
            dpte_reg_fx = dPTE_DICS_fit.params[f"dPTE:C(Reg, Treatment('L8143-rh'))[T.{lab}]:Task[T.task]"]
            vals[idx] += dpte_reg_fx
...
fig.suptitle("dPTE predicts power, high alpha band", fontsize=70)
...
# condition
formula = "DICS_log ~ dPTE*C(Reg, Treatment('L8143-rh'))*C(Cond, Treatment('rest'))"
model = MixedLM.from_formula(formula, df, groups=df['Subj'],
                             re_formula=None)
dPTE_DICS_fit = model.fit(method=["powell", "lbfgs"])
print(dPTE_DICS_fit.summary())
...
for cond_idx, cond in enumerate(["audio", "visual", "visselten"]):
    if dPTE_DICS_fit.pvalues[f"dPTE:C(Cond, Treatment('rest'))[T.{cond}]"] < .05:
        fx_task = dPTE_DICS_fit.params[f"dPTE:C(Cond, Treatment('rest'))[T.{cond}]"]
    else:
        fx_task = 0
    vals = np.zeros(len(labels))
    for idx, label in enumerate(labels):
        ...
        vals[idx] = intercept + fx + fx_task
        if lab != "L8143-rh":
            if dPTE_DICS_fit.pvalues[f"C(Reg, Treatment('L8143-rh'))[T.{lab}]:C(Cond, Treatment('rest'))[T.{cond}]"] < .05:
                reg_fx = dPTE_DICS_fit.params[f"C(Reg, Treatment('L8143-rh'))[T.{lab}]:C(Cond, Treatment('rest'))[T.{cond}]"]
                vals[idx] += reg_fx
            if dPTE_DICS_fit.pvalues[f"dPTE:C(Reg, Treatment('L8143-rh'))[T.{lab}]:C(Cond, Treatment('rest'))[T.{cond}]"] < .05:
                dpte_reg_fx = dPTE_DICS_fit.params[f"dPTE:C(Reg, Treatment('L8143-rh'))[T.{lab}]:C(Cond, Treatment('rest'))[T.{cond}]"]
                vals[idx] += dpte_reg_fx
...
scalmap = cm.ScalarMappable(norm, "seismic")
...
ax.set_xlabel("Estimated coefficient", fontsize=70)
