...
for sub in subjs:
    for run_idx,run in enumerate(runs):
        ...
        icaref.save("{dir}nc_{sub}_{run}_hand_ref-ica.fif".format(dir=proc_dir,sub=sub,run=run))
        icameg = mne.preprocessing.ICA(n_components=None,max_iter=1000,method="picard")
        picks = mne.pick_types(raw.info,meg=True,ref_meg=False)
        icameg.fit(raw,picks=picks)
        icameg.save("{dir}nc_{sub}_{run}_hand_meg-ica.fif".format(dir=proc_dir,sub=sub,run=run))
