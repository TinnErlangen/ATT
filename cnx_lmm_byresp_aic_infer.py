...
var_base = "RT:C(Block, Treatment('audio'))" # stem of the condition names in statsmodels format
...
if calc_aic:
    ...
    for mod in models:
        for n_idx in range(node_n):
            print(n_idx)
            try:
                this_mod = MixedLMResults.load("{}{}/{}_reg70_byresp_lmm_{}{}.pickle".format(proc_dir,band,mod,n_idx,z_name))
            except:
                continue
    ...
    for n_idx in range(node_n):
        ...
        if aic_comps["threshed"][n_idx].sum() == 1: # all other models significantly different than best model
            ...
            if aic_comps["threshed"][n_idx][2] == 1: # if the best model was "cond," than find out which conditions were significantly different
            ...
            elif aic_comps["threshed"][n_idx][1] == 1: # best model was simple
    ...
    with open("{}{}/byresp_aic{}.pickle".format(proc_dir,band,z_name), "wb") as f:
        pickle.dump(aic_comps,f)
else:
    with open("{}{}/byresp_aic{}.pickle".format(proc_dir,band,z_name), "rb") as f:
...
for n_idx in range(node_n):
    ...
    if aic_comps["dual_winner"][n_idx]:
        cnx_params["simple_RT"][triu_inds[0][n_idx],triu_inds[1][n_idx]] = aic_comps["simple_sig_params"][n_idx]
...
for stat_cond,cond in zip(stat_conds,conds):
    ...
    if write_images:
        write_brain_image(cond+"_rt", views, params_brains[-1], "../images/")

params_brains.append(plot_directed_cnx(cnx_params["simple_RT"],labels,parc,
                     alpha_min=None,alpha_max=None,
                     ldown_title="Simple (task)", top_cnx=top_cnx))
if write_images:
    write_brain_image("simple_rt", views, params_brains[-1], "../images/")
...
if write_images:
    write_brain_image("rainbow_rt", views, params_brains[-1],
                "../images/")
