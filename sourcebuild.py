...
for k,v in mri_key.items():
    ...
    # src = mne.setup_source_space(k,surface="white", spacing=spacing,
    ...
    for fwd in fwds:
        mne.write_forward_solution("{dir}nc_{sub}_{run}_{spacing}-fwd.fif".format(
                                    dir=proc_dir,sub=v,run=run,spacing=spacing), fwd,
                                    overwrite=True)
    ...
    # sens_lh.save("{dir}nc_{sub}_{spacing}".format(dir=proc_dir,sub=v,spacing=spacing))
    # sens_rh.save("{dir}nc_{sub}_{spacing}".format(dir=proc_dir,sub=v,spacing=spacing))

        sen = mne.sensitivity_map(fwd,ch_type="mag",mode="fixed")
        sen.data[sen.data<thresh] = 0
        sens_lh, sens_rh = mne.stc_to_label(sen,src=src,subjects_dir=subjects_dir,smooth=True)
        sens_lh.save("{dir}nc_{sub}_{run}_{spacing}".format(dir=proc_dir,sub=v,run=run,spacing=spacing))
        sens_rh.save("{dir}nc_{sub}_{run}_{spacing}".format(dir=proc_dir,sub=v,run=run,spacing=spacing))
