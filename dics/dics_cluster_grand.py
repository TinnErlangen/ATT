...
n_jobs = 4
f_range = [4,30]
f_list = list(np.arange(f_range[0],f_range[1]+1))
f_subset = [14,30]
#f_subset = None
...
if f_subset:
    fs = [f_list.index(f_subset[0]),f_list.index(f_subset[1])]
    X = [x[:,fs[0]:fs[1],].mean(axis=1,keepdims=True) for x in X]
...
if len(conds)==2:
    ...
    sign = np.sign(x_arr[:4,].mean(axis=0)-x_arr[4:,].mean(axis=0))
    raw_f.data *= sign.T
raw_f.save("{}stc_f_{}-{}Hz_{}_{}_{}".format(proc_dir, f_subset[0], f_subset[1], cond_str,
                                                 thresh_str, effects[effect_idx]))
with open("{}clu_{}-{}Hz_{}_{}_{}".format(proc_dir, f_subset[0], f_subset[1], cond_str,
...
try:
    ...
    stc_clu.save("{}stc_clu_{}-{}Hz_{}_{}_{}".format(proc_dir, f_subset[0], f_subset[1],
                                                     cond_str,
                                                     thresh_str,
                                                     effects[effect_idx]))
except:
