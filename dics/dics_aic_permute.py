...
if recalc:
    ...
    np.save("{}dics.npy".format(proc_dir), data)
    dm.to_pickle("{}dics_dm.pickle".format(proc_dir))
    dm_simple.to_pickle("{}dics_dm_simple.pickle".format(proc_dir))
else:
    data = np.load("{}dics.npy".format(proc_dir))
    dm = pd.read_pickle("{}dics_dm.pickle".format(proc_dir))
    dm_simple = pd.read_pickle("{}dics_dm_simple.pickle".format(proc_dir))


group_id = dm["Subj"]
aics = {"simple":np.zeros((len(regions), perm_n)),
        "cond":np.zeros((len(regions), perm_n))}
for perm_idx in range(perm_n):
    ...
    for reg_idx in range(len(data)):
        ...
        dm_temp = shuffle_df(dm_temp)
        dm_simple_temp = shuffle_df(dm_simple_temp)
        ...
        aics["simple"][reg_idx,perm_idx] = mod_fit.aic

        formula = "Brain ~ C(Block, Treatment('rest'))"
        model = MixedLM.from_formula(formula, dm_temp, groups=group_id)
        mod_fit = model.fit(reml=False)
        aics["cond"][reg_idx,perm_idx] = mod_fit.aic

with open("{}dics_aic_perm.pickle".format(proc_dir), "wb") as f:
    pickle.dump(aics, f)
